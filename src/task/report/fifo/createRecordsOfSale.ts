...
export const createRecordsFromSale = (tx: Transaction, fifoQueue: FifoQueue): FifoSaleResult => {
  ...
  const feePerItem = tx.taxcur_fee / tx.item_count
  ...
  while (remainingCount > 0 && fifoQueue.hasNextBuy()) {
    ...
    if (currentBuy.remaining_item_count <= remainingCount) {
      ...
      const sellCost = multiply(currentBuy.remaining_item_count, tx.taxcur_price_per_item)

      // sellCost - currentBuy.total_cost
      const profitOrLoss = subtract(sellCost, currentBuy.taxcur_remaining_cost)

      // currentBuy.item_count * (tx.cur_fee / tx.item_count)
      const buyingFee = multiply(
        currentBuy.remaining_item_count,
        divide(tx.taxcur_fee, tx.item_count),
      )
      ...
      fifoRecords.push({
        ...
        taxcur_sell_cost_per_item: parseFloat(tx.taxcur_price_per_item.toFixed(10)),
        taxcur_buy_cost_per_item: parseFloat(currentBuy.taxcur_price_per_item.toFixed(10)),
        taxcur_original_buy_cost: parseFloat(currentBuy.taxcur_remaining_cost.toFixed(4)),
        taxcur_profit: parseFloat(profitOrLoss.toFixed(4)),
        taxcur_buying_fee: buyingFee,
        taxcur_selling_fee: sellingFee,
        taxcur_total_fee: parseFloat((buyingFee + sellingFee).toFixed(4)),
      })

      remainingCount -= currentBuy.remaining_item_count
      currentBuy.remaining_item_count = 0
      currentBuy.taxcur_remaining_cost = 0
      ...
      const partialSellCost = remainingCount * tx.taxcur_price_per_item
      const partialBuyCost = remainingCount * currentBuy.taxcur_price_per_item
      const profitOrLoss = partialSellCost - partialBuyCost

      const buyingFee = parseFloat(
        (remainingCount * (tx.taxcur_fee / tx.item_count)).toFixed(4),
      )
      ...
      fifoRecords.push({
        ...
        taxcur_sell_cost_per_item: parseFloat(tx.taxcur_price_per_item.toFixed(10)),
        taxcur_buy_cost_per_item: parseFloat(currentBuy.taxcur_price_per_item.toFixed(10)),
        taxcur_original_buy_cost: parseFloat(partialBuyCost.toFixed(4)),
        taxcur_profit: parseFloat(profitOrLoss.toFixed(4)),
        taxcur_buying_fee: buyingFee,
        taxcur_selling_fee: sellingFee,
        taxcur_total_fee: parseFloat((buyingFee + sellingFee).toFixed(4)),
      ...
      currentBuy.taxcur_remaining_cost = subtract(currentBuy.taxcur_remaining_cost, partialBuyCost)
      remainingCount = 0
      ...
      buyUpdates[currentBuy._id] = {
        ...
        remaining_cost: currentBuy.taxcur_remaining_cost,
      }
